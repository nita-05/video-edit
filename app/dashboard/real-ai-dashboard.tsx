...
function RealAIDashboard() {
  ...
  const connectPlatform = async (platformId: string) => {
    try {
      ...
      const res = await fetch(`\`\${API_BASE_URL}\`/api/vport/connect/${endpoint}`);
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      try {
        const r = await fetch('\`\${API_BASE_URL}\`/api/vport/status/youtube');
  ...
  const schedulePost = async (platformId: string, content: any) => {
    try {
      const response = await fetch('\`\${API_BASE_URL}\`/api/vport/schedule', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleAIMerge = async () => {
    ...
    try {
      ...
      for (const clip of toUpload) {
        ...
        const up = await fetch('\`\${API_BASE_URL}\`/api/upload', { method: 'POST', body: formData });
        if (!up.ok) {
    ...
    try {
      ...
      const performMerge = async (attempt: number): Promise<void> => {
        try {
          const res = await fetch('\`\${API_BASE_URL}\`/api/ai/merge', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
                             ...
                             <button
                               ...
                               onClick={async () => {
                                 ...
                                 try {
                                   ...
                                   const response = await fetch(`\`\${API_BASE_URL}\`/api/vport/publish/${platform.id}`, {
                                     method: 'POST',
                                     headers: { 'Content-Type': 'application/json' },
                                     body: JSON.stringify({
